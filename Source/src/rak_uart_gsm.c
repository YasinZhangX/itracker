 ...
 *********************************************************************************************************
 */
void delay_ms_hard(uint32_t ms)
{
	nrf_delay_ms(ms);
}
...
void Gsm_PowerUp(void)
{
	...
	GSM_PWR_ON;
	GSM_RESET_HIGH;
	delay_ms_hard(60);           // >30ms

	/*Pwr key keep to low at least 100ms*/
	GSM_PWRKEY_LOW;
	delay_ms_hard(300); //300ms
	GSM_PWRKEY_HIGH;
	delay_ms_hard(500);
}
...
int Gsm_nb_iot_config(void)
{
	...
	if (doflag == 0) {
		...
		memset(GSM_RSP, 0, GSM_GENER_CMD_LEN);
		retval = Gsm_WaitRspOK((char *)GSM_RSP, GSM_GENER_CMD_TIMEOUT * 60, true);
		if (retval == -1) {
			Gsm_print("AT+QIGETERROR");
			memset(GSM_RSP, 0, GSM_GENER_CMD_LEN);
			Gsm_WaitRspOK((char *)GSM_RSP, GSM_GENER_CMD_TIMEOUT * 5, true);
...
void gps_data_get()
{
	...
	do {
		retry_count--;
		retval = Gsm_WaitRspOK((char *)GSM_RSP, GSM_GENER_CMD_TIMEOUT * 10, true);
		if (retval >= 0 && strstr((char *)GSM_RSP, "QGPSGNMEA")) {
			GpsParseGpsData_2((int8_t *)GSM_RSP);
...
{
	int retval = MQTT_CODE_ERROR_NETWORK;
	char *cmd;
	int retry_count;
...
int Gsm_ReadRecvBufferCmd(void *context, uint16_t len, uint32_t timeout)
{
	...
	char *cmd;
	uint8_t connectID;
	static uint8_t enterFlag = FALSE;
...
//	if (enterFlag) {
//		Gsm_print("AT");
//		Gsm_WaitRspOK(NULL, GSM_GENER_CMD_TIMEOUT * 2, TRUE);
//		enterFlag = FALSE;
//	}
	if (len == 228 || len == 512)
		enterFlag = TRUE;
